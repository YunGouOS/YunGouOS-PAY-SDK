...
try {
    ...
    switch ($apitype) {
        case "native":
            //扫码支付
            $result = $wxpay->nativePay($out_trade_no, $total_fee, $mch_id, $body, $type,$app_id, $attach, $notify_url, null, null, null, null,$key);
        ...
        case "cashier":
            //收银台支付
            $result = $wxpay->cashierPay($out_trade_no, $total_fee, $mch_id, $body,$app_id, $attach, $notify_url, $return_url, null, null, null,null, $key);
        ...
        case "wxpay_h5":
            //微信H5支付
            $result = $wxpay->wapPay($out_trade_no, $total_fee, $mch_id, $body, $app_id,$attach, $notify_url, $return_url, null, null, null, null,$key);
        ...
        case "wxpay_app":
            ...
            $result = $wxpay->appPay($appId, $out_trade_no, $total_fee, $mch_id, $body, $app_id,$attach, $notify_url, null, null, null,null, $key);
            //返回拉起APP支付的参数，需要APP客户端遵循微信官方规则进行拉起支付
            break;
        case "alipay_native":
            //扫码支付
            $result = $alipay->nativePay($out_trade_no, $total_fee, $mch_id, $body, $type, $app_id, $attach, $notify_url,null,null,null,null,null, $key);
            //此处type传递的是2 所以返回的是支付二维码的地址直接显示即可。如果传递1 返回的是支付宝原生的二维码支付连接，需要自己写生成二维码图片的逻辑
            $html = '<img src="' . $result . '"/>';
            echo $html;
            break;
        case "alipay_wap":
            $result = $alipay->wapPay($out_trade_no, $total_fee, $mch_id, $app_id, $body, $attach, $notify_url,null,null,null,null,null, $key);
